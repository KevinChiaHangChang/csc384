...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def getMax(gameState, currDepth):
          ...
          for each_action in actions:
            successorState = gameState.generateSuccessor(0,each_action)
            maxScore = max(maxScore, getRand(successorState,1,currDepth))
          return maxScore
        
        # model ghosts as choosing uniformly at random from their legal moves
        def getRand(gameState, agentIndex, currDepth):
          # determine if terminal node
          if gameState.isWin() or gameState.isLose() or currDepth == 0:
            return self.evaluationFunction(gameState)
          randScore = 0
          actions = gameState.getLegalActions(agentIndex)
          probMove = 1.0/len(actions)
          # if reach last agent index, need to decrement depth by 1
          if agentIndex+1 == gameState.getNumAgents():
            for each_action in actions:
              successorState = gameState.generateSuccessor(agentIndex,each_action)
              randScore += probMove*getMax(successorState,currDepth-1)
          else:
            for each_action in actions:
              successorState = gameState.generateSuccessor(agentIndex,each_action)
              randScore += probMove*getRand(successorState,agentIndex+1,currDepth)
          return randScore
        ...
        for each_action in actions:
            successorState = gameState.generateSuccessor(0,each_action)
            prevScore = maxScore
            maxScore = max(maxScore, getRand(successorState,1,self.depth))
            if maxScore > prevScore:
                optimal_action = each_action
        return optimal_action
...
def betterEvaluationFunction(currentGameState):
    ...
    # find min distance to food
    minDist = float('inf')
    for eachFood in currFood:
      tmpDist = manhattanDistance(currPos,eachFood)
      minDist = min(minDist,tmpDist)
      # find min distance to ghost
    minGhost = float('inf')
    for eachGhost in currentGameState.getGhostStates():
      tmpDist = manhattanDistance(currPos,eachGhost.getPosition())
      minGhost = min(minGhost,tmpDist)
    # find min distance to capsule
    minCapsule = float('inf')
    for eachCapsule in currCapsules:
      tmpDist = manhattanDistance(currPos,eachCapsule)
      minCapsule = min(minCapsule,tmpDist)
    # score += -1.5*minDist
    # score += -4.0*(len(currFood))
    # score += -2.0*(1.0/minGhost)
    # assign weights
    score += 3.5/minDist
    score += 1.5/minCapsule
    if len(currFood) == 0:
      score += 20000.0
    else:
      score += 10.0/len(currFood)
    if len(currCapsules) == 0:
      score += 10000.0
    else:
      score += 5.0/len(currCapsules)
    if minGhost == 0:
      score = -(float('inf'))
    # only consider ghost distance is within radius of 3.0
    elif minGhost < 3.0:
      score += 2.0/minGhost
